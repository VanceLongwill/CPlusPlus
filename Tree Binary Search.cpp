...
int bul (node * agac,int aranan) // find 
{
	if(agac ==NULL)
		return -1;
	if(agac->data == aranan)
		return 1;	
	if(bul(agac->sag,aranan)==1) // herhangi bir yerde buldu
		return 1;
...
int main()
{
	...
	printf("max = %d mid= %d",max(agac),min(agac));	
	agac = sil(agac,190);
	dolas(agac);
